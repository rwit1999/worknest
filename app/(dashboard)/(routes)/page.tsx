...
const DashboardHomePage = async () => {
  ...
  const jobs = await db.job.findMany({  // Use db.Job as per the schema
    include: {
      company: true,  // This includes the company data for each job
    },
  });

  const categories = await db.category.findMany({  // Use db.Category as per the schema
    orderBy: {
      name: 'asc',
    },
  });

  const companies = await db.company.findMany({  // Use db.Company as per the schema
    orderBy: {
      createdAt: 'desc',
    },
  });
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {companies.map((company) => (
            <div key={company.id} className="p-6 bg-white rounded-lg shadow-md">
              ...
              <Image src={company.logo || '/default-company-logo.png'} alt={company.name} width={100} height={100} />
            </div>
          ))}
        </div>
      </div>
